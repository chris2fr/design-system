...
- ajout d'un attribut `data-webfacile-group="false"` pour dissocier le comportement d'ouverture/fermeture des accordéons à l'intérieur d'un groupe d'accordéons
...
- ajout de l'attribut `data-webfacile-action` permettant d'activer l'envoi d'action sur un élément particulier et de spécifier la partie title de l'actionName envoyé
...
- ajoute la classe utilitaire `.webfacile-no-print` pour retirer un élement de l'impression
...
- Le bouton FranceConnect doit répondre aux critères d’accessibilité qui redéfinissent le letter-spacing et la taille de fonte.
  ...
  - Retrait du '+' de 'FranceConnect+' dans l'intitulé de `webfacile-connect__brand`. Celui-ci est désormais placé en contenu du pseudo-élément after du bouton
...
- retire 1v de padding gauche et droite sur `webfacile-sidemenu__inner` en desktop
...
- passe les liens .webfacile-footer__content-link en $text-default-grey
- passe le padding top de .webfacile-footer__bottom-list à 4v
- correction des espacements autour de webfacile-footer-body :  en mobile et en desktop (32px en haut et 24px en bas)
- titre “nos partenaire“ → webfacile-footer__partners-title passe en graisse régular, couleur text-default-grey
- ecart de 12px sous “Nos partenaire” en mobile/desktop
- enleve le padding sur .webfacile-footer__partners .webfacile-footer__logo, ajoute une border 1px en $border-default-grey + un background en background-default-grey
...
- ajoute un margin-top: -1.25rem (-20px) sur le webfacile-mega-menu__leader
- passe le texte de description et le lien du webfacile-mega-menu__leader en taille sm
- supprime la classe webfacile-mb-4v de la colonne entourant le webfacile-mega-menu__leader
- le texte du bouton de navigation passe en $text-action-high-webfacile-primary à l'ouverture


#### 🐛 fix(unavailable): couleur de fond et image décorative [(#787)](https://github.com/GouvernementFR/dsfr/pull/787)
- change la couleur de fond de page pour du $background-alt-webfacile-primary
...
- retire la marge négative sur le modifier .webfacile-btn--close et corrige l'impact sur les composants 'alert', 'navigation', et 'notice'
...
- place le bouton d'action dans un groupe de bouton webfacile-btns-group--inline-md sur les exemples "Lettre d'info seule" et "Réseaux sociaux et Lettre d'info mise en avant" pour que le bouton prenne l’ensemble de la largeur en vue mobile.


#### 🐛 fix(tag): token de couleur du tag selectionnable [(#780)](https://github.com/GouvernementFR/dsfr/pull/780)
- utilisation du token $text-inverted-webfacile-primary sur la couleur du texte des tag selectionnable et supprimable à la place de $text-inverted-grey
- remplacement du token de couleur de fond des tag selectionnable par $background-active-webfacile-primary au lieu de $background-action-high-webfacile-primary
...
- corrige la largeur des class webfacile-fieldset__content pour la version dépréciée


#### 🐛 fix(checkbox): met a jour le token de la coche [(#762)](https://github.com/GouvernementFR/dsfr/pull/762)
- passe la couleur de la coche en $text-inverted-webfacile-primary
...
- ajout de l'attribut data-webfacile-analytics-rating qui permet de mesurer le taux d'interaction sur un élément particulier
...
- BREAKING CHANGE : il est nécessaire d'ajouter les attributs `data-webfacile-valid`et `data-webfacile-error` avec les textes correspondants à l'état (respectivement, en français, "validé" et "en erreur"
...
- `.webfacile-tile--no-border` sans le bordure encadrant la tuile (mais la barre épaisse basse reste)
- `.webfacile-tile--shadow` avec élévation
- `.webfacile-tile--grey` en gris contrast
- `.webfacile-tile--no-background` couleur de fond transparente
...
- Uniformisation du menu latéral, navigation, et accordéon
  - ajout d'un fond open-webfacile-primary et du texte en webfacile-primary sur les boutons d'ouverture en état ouvert
...
- Corrige le style du bouton mobile en action-high-webfacile-primary (cohérence navigation/accordion)
...
- conditionne l'ajout de la classe utile `webfacile-icon` à l'utilisation d'une string en paramètre `icon`
...
- ajoute une condition sur l'ajout du wrapper `.webfacile-btns-group` dans la template `ejs`
...
Actuellement la bordure gauche montrant l'état d'erreur/succès/info n'est appliqué que dans le cas d'un groupe de champ en erreur via les modificateur .webfacile-fieldset--error, .webfacile-fieldset--valid, .webfacile-fieldset--info

Afin d'être ISO avec l'UI nous rajoutons cet élément visuel sur : 
- les champs seuls (.webfacile-input-group) : 
  - `.webfacile-input-group--error`
  - `.webfacile-input-group--valid`
  - `.webfacile-input-group--info`
- les selects (.webfacile-select-group)
  - `.webfacile-select-group--error`
  - `.webfacile-select-group--valid`
  - `.webfacile-select-group--info`
...
- Correctif typo sur la classe `webfacile-grid-row--gutters`
...
- Passage icône et intitulé en action-high-webfacile-primary
- Ajout background-open-webfacile-primary sur le bouton lorsque l'élément est ouvert
...
- `.webfacile-raw-link` retire toute stylisation sur les liens natifs
- `.webfacile-reset-link` remet le soulignement natif du navigateur (`text-decoration: underline;`)
...
Le snippet :
```html
<div class="webfacile-radio-group webfacile-radio-rich">
  <input value="1" type="radio" id="radio-rich-1" name="radio-rich">
  <label class="webfacile-label" for="radio-rich-1">
      Libellé bouton radio
  </label>
  <div class="webfacile-radio-rich__img">
...
<div class="webfacile-radio-group webfacile-radio-rich">
  ...
  <div class="webfacile-radio-rich__pictogram">
      <svg aria-hidden="true" class="webfacile-artwork" viewBox="0 0 80 80" width="80px" height="80px">
          <use class="webfacile-artwork-decorative" href="../../../dist/artwork/pictograms/buildings/city-hall.svg#artwork-decorative"></use>
          <use class="webfacile-artwork-minor" href="../../../dist/artwork/pictograms/buildings/city-hall.svg#artwork-minor"></use>
          <use class="webfacile-artwork-major" href="../../../dist/artwork/pictograms/buildings/city-hall.svg#artwork-major"></use>
      </svg>
  </div>
</div>
```
...
BREAKING CHANGE : `webfacile-radio-rich__img` devient `webfacile-radio-rich__pictogram`
...
Les tuiles peuvent maintenant être de type téléchargement (comme les cartes)
  ...
  - Le détail de la tuile de téléchargement est obligatoire et il peut être rempli automatiquement en fonction du fichier à télécharger en plaçant à l'attribut "data-webfacile-assess-file" sur le lien (comme pour carte)

Les tuiles ont maintenant par défaut une icone.
  - arrow-right (par défaut)
  - external-link (en target="_blank")
  - download (avec la classe webfacile-tile--download)

Les tuiles désactivées (a sans href) ont à présent:
  - la bordure bottom en grise
  - l'icone et le titre en gris
  
Ajout des classes "webfacile-tile--vertical@md" et  "webfacile-tile--vertical@lg" pour passer une tuile horizontale, ou download, en vertical à partir des breakpoints md et lg
...
- Ajout d'un niveau d'encapsulation dans la structure html
  - Ajout d'un wrapper "webfacile-tile__content" pour englober le contenu
  - Ajout d'un wrapper "webfacile-tile__header" pour englober l'image
- L'image des tuiles est remplacée par un pictogramme
  - La classe "webfacile-tile__img" devient "webfacile-tile__pictogram" 
  - Son contenu est maintenant un svg "webfacile-artwork"
- Ajout de la possibilité de placer un badge, un tag, un texte de détail, dans le contenu de la tuile
- Ajout d'une taille de tuile SM : "webfacile-tile--sm"

**⚠️ Breaking Change**
Le snippet de code d'une tuile : 
```html
<div class="webfacile-tile webfacile-enlarge-link">
    <div class="webfacile-tile__body">
        <h4 class="webfacile-tile__title">
            <a class="webfacile-tile__link" href>Titre M bold</a>
        </h4>
        <p class="webfacile-tile__desc">Texte M regular 2 lignes max</p>
   </div>
   <div class="webfacile-tile__img">
       <img class="webfacile-responsive-img" src="../../../example/img/placeholder.1x1.png" alt="" />
...
Devient : 
```html
<div class="webfacile-tile webfacile-enlarge-link" id="tile-6584">
    <div class="webfacile-tile__body">
        <div class="webfacile-tile__content">
            <h3 class="webfacile-tile__title">
                <a href="#">Intitulé de la tuile</a>
            </h3>
            <p class="webfacile-tile__desc">Lorem [...] elit ut.</p>
        </div>
    </div>
    <div class="webfacile-tile__header">
        <div class="webfacile-tile__pictogram">
            <svg aria-hidden="true" class="webfacile-artwork" viewBox="0 0 80 80" width="80px" height="80px">
                <use class="webfacile-artwork-decorative" href="../../../dist/artwork/pictograms/buildings/city-hall.svg#artwork-decorative"></use>
                <use class="webfacile-artwork-minor" href="../../../dist/artwork/pictograms/buildings/city-hall.svg#artwork-minor"></use>
                <use class="webfacile-artwork-major" href="../../../dist/artwork/pictograms/buildings/city-hall.svg#artwork-major"></use>
...
Ajout de la variante de carte : webfacile-card--download 
Ajout de la variante de lien : webfacile-link--download 

Le lien de téléchargement : 
```html
<div class="webfacile-download">
    <h3>
        <a href="[À MODIFIER - /example/img/image.jpg]" download class="webfacile-download__link">Télécharger le document lorem ipsum sit dolores amet
            <span class="webfacile-download__detail">
                JPG – 61,88 ko
            </span>
        </a>
    </h3>
...
DEVIENT : 
```html
<a class="webfacile-link webfacile-link--download" download href="[À MODIFIER - example/img/image.jpg]">
    Télécharger le document lorem ipsum sit dolores amet <span class="webfacile-link__detail">JPG – 61,88 ko</span>
</a>
```

et la carte de téléchargement : 
```html
<div class="webfacile-download webfacile-enlarge-link webfacile-download--card">
    <h3>
        <a href="[À MODIFIER - example/img/image.jpg]" download class="webfacile-download__link">Télécharger le document lorem ipsum sit dolores amet
            <span class="webfacile-download__detail">
                JPG – 61,88 ko
    ...
    <p class="webfacile-download__desc">Texte de description<</p>
</div>
```
DEVIENT : 
```html
<div class="webfacile-card webfacile-enlarge-link webfacile-card--download">
    <div class="webfacile-card__body">
        <div class="webfacile-card__content">
            <h3 class="webfacile-card__title">
                <a download href="[À MODIFIER - example/img/image.jpg]">
                    Télécharger le document lorem ipsum sit dolores amet
                </a>
            </h3>
            <p class="webfacile-card__desc">Texte de description</p>
            <div class="webfacile-card__end">
                <p class="webfacile-card__detail">JPG – 61,88 ko</p>
            </div>
        </div>
    </div>
</div>
...
BREAKING CHANGE : Remplacer le composant webfacile-download par la variante de link webfacile-link--download et remplacer le composant webfacile-download--card par une carte webfacile-card--download
...
L'ajout de l'attribut `data-webfacile-prevent-conceal` permet de préserver un lien ou un bouton particulier de ce nouveau comportement.
...
webfacile--arrow-right-up-circle-fill,
webfacile--arrow-right-down-circle-fill,
webfacile--equal-circle-fill,
...
#### refactor(form): Ajout des webfacile-control et correction des pattern civility & name [(#401)](https://github.com/GouvernementFR/dsfr/pull/401)
...
#### fix(icons): correctif depreciation webfacile-fi-checkbox-circle-line [(#334)](https://github.com/GouvernementFR/dsfr/pull/334)
...
#### refactor(core, card, content): ajout utilitaire webfacile-ratio et aspect-ratio des content img & vid [(#316)](https://github.com/GouvernementFR/dsfr/pull/316)
...
#### fix(dswebfacile-deprecated): correction du nom de fichier sur le js legacy [(#295)](https://github.com/GouvernementFR/dsfr/pull/295)
...
#### fix: bouton sidemenu display none / scheme package.yml / webfacile-input code inutile / table refactor et couleurs [(#100)](https://github.com/GouvernementFR/dsfr/pull/100)


#### fix(logo): correction webfacile-primary & mourning [(#99)](https://github.com/GouvernementFR/dsfr/pull/99)
...
#### fix(core): webfacile-text devient webfacile-text--md [(#63)](https://github.com/GouvernementFR/dsfr/pull/63)


#### fix(core): reset bordure des inputs (ios) [(#58)](https://github.com/GouvernementFR/dsfr/pull/58)


#### fix(search): correction webfacile-input--error et webfacile-input--valid [(#57)](https://github.com/GouvernementFR/dsfr/pull/57)
...
#### fix(input): webfacile-input--error sur un textarea le passe en rouge [(#47)](https://github.com/GouvernementFR/dsfr/pull/47)
...
#### fix(core): correction utilitaire taille d'icone webfacile-fi--size
ajout taille webfacile-fi--md
