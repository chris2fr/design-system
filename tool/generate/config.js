...
const analyse = (id, path, ascendants = []) => {
  ...
  config.dist = data.dist ? data.dist : config.path.replace('src/designsystem', 'dist').replace(data.id, data.filename || data.id);

  if (config.standalone) {
    config.standalone.dist = config.path.replace('src/designsystem', 'standalone').replace(data.id, data.filename || data.id);
...
const generateJSON = () => {
  const config = analyse('designsystem', 'src/designsystem');
...
const generateConfig = async () => {
  generateCore();
  await generateIcon('src/designsystem/core/icon');
  await generatePictogram('src/designsystem/core/asset/artwork/pictograms');
